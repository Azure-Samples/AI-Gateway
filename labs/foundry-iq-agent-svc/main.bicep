...
@description('KB Name for AI Search')
param knowledgeBaseName string = 'default-kb'
...
// 11. RBAC: Grant AI Foundry project managed identity "Cognitive Services OpenAI User" on AI Services (for vectorizer)
...
// 12. Create MCP pass-through API in APIM to route to AI Search KB MCP endpoint, with authentication via APIM-managed identity
module mcpPassThroughModule './src/mcp-server/pass-trought.bicep' = {
  name: 'mcpPassThroughModule'
  params: {
    apimServiceName: apimModule.outputs.name
    backendName: 'kb-mcp-backend'
    backendDescription: 'Backend for KB MCP Server'
    backendURL: 'https://${searchService.name}.search.windows.net/knowledgebases/${knowledgeBaseName}/mcp'
  }
}

//13. Create AI Search connection in AI Foundry project - this enables the use of UI to explore existing KBs in AI Search
// it's not needed for the agent to use the KB, but it allows Foundry UI users to easily explore and test with the KB using the Foundry UI

resource aiFoundryProject 'Microsoft.CognitiveServices/accounts@2025-06-01' existing = {
...
output kbMcpEndpoint string = mcpPassThroughModule.outputs.apiURL
